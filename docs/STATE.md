...
**HEAD:** `17e777b`, дерево чистое, совпадает с `origin/master` (8 сентября 2026).
...
9, 10, 11 закрыты в `683a2a0`; 13 — в `29f8c99`).
...
**35. `TemplateState.waiting_for_instruction` без fallback.**
Хендлер объявлен как `@admin_router.message(TemplateState.waiting_for_instruction,
F.text)`. Стикер, фото или голосовое на этом экране не совпадают ни с одним
хендлером: бот молчит, состояние остаётся. Та же форма, что 14.

Соседний `on_example_received` фильтра не имеет и на сообщение без текста
отвечает `EXAMPLE_EMPTY_TEXT` — два экрана подряд ведут себя по-разному.
Чинится снятием `F.text` и разбором пустого ввода внутри хендлера, как в
примерах. Появилось вместе с шаблонами постов (`17e777b`).
→ [post-templates.md](post-templates.md#приём-примера)
...
Этапы 1 и 2 сделаны (слой данных, привязка хранилища, шаблоны постов), этапы 3–5
не начаты. Что именно осталось — в [post-type-material.md](post-type-material.md)
и [post-templates.md](post-templates.md).
...
3. Просмотр текста сохранённых примеров шаблона. Сейчас в списке видны только
   номер и длина: примеры хранятся как Telegram HTML, а бот шлёт всё с
   `parse_mode=HTML`, так что показ содержимого требует экранирования, а
   разметка в превью нечитаема. Нужен отдельный экран «показать пример #N».
4. Видео и документы в собственных постах, включая смешанные альбомы.
5. Канал-хранилище для картинок на случай смены токена (`file_id` привязан к токену).
6. `RoleCache` на Redis.
7. mypy в pre-commit, README.
