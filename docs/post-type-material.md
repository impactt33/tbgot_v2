...
**Статус: сделан только слой данных (этап 1). Этапы 2–5 не начаты, план не
согласован.** Постов этого типа бот пока не создаёт.
...
2. **Привязка хранилища** в мастере добавления канала: `request_chat`, проверки
   «бот админ», `can_post_messages` и наличие `username`, кнопка Skip, отдельная
   перепривязка.
...
**Открытые вопросы, блокирующие этап 2:**
- поддерживать ли `video`/`audio` кроме `document`;
- ссылка в тексте или в кнопке (сейчас `MaterialPayload` предполагает текст).
