...
**Статус: сделан слой данных (этап 1) и привязка хранилища (первая половина
этапа 2). Этапы 3–5 не начаты.** Постов этого типа бот пока не создаёт.
...
## Этап 2 — привязка хранилища: сделано

Мастер канала спрашивает хранилище сразу после подключения канала, плюс
отдельный вход «Set up channel» для каналов, добавленных раньше. Пикер отсекает
приватные каналы флагом `chat_has_username`, права проверяются через
`get_chat_member`, пропуск — нормальный исход. Подробности —
[users-and-channels.md](users-and-channels.md#мастер-канала).

Осталась вторая половина этапа: **примеры постов**, по которым модель будет
воспроизводить формат канала. Решено хранить не выведенный «шаблон», а
**примеры плюс инструкцию**:

- `examples: list[str]` — 3–5 реальных постов канала этого типа, как есть;
- `instruction: str | None` — свободный текст от админа, на пару
  **(канал, тип поста)**, а не на канал целиком.

Few-shot воспроизводит тон и структуру точнее, чем описание структуры словами, и
не требует ни этапа вывода шаблона, ни UI для его правки. Таблицу делать общей,
но подключать сначала только к `MATERIAL`: иначе одна задача трогает работающие
`generate_quiz` и `generate_source`.
...
4. **Тип поста:** `CreateMaterialPostUseCase`, `TelegramPublisher._publish_material`,
